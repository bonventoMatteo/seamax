...
export default function Fleet({ isNight }: FleetProps) {
  ...
  // 2. Cores Dinâmicas baseadas no isNight
  const theme = {
    bg: isNight ? 'bg-zinc-950' : 'bg-white',
    text: isNight ? 'text-white' : 'text-zinc-950',
    cardBg: isNight ? 'bg-zinc-900/40 border-white/5' : 'bg-white border-zinc-100',
    cardText: isNight ? 'text-zinc-100' : 'text-zinc-800',
    muted: isNight ? 'text-zinc-500' : 'text-zinc-400',
    footer: isNight ? 'bg-zinc-900/60 border-white/5' : 'bg-zinc-50 border-zinc-100',
    grid: isNight ? 'bg-[radial-gradient(#3f3f46_1px,transparent_1px)]' : 'bg-[radial-gradient(#e5e7eb_1px,transparent_1px)]',
    line: isNight ? 'bg-zinc-800' : 'bg-zinc-100',
  };

  return (
    <section ref={sectionRef} className={`py-32 relative overflow-hidden transition-colors duration-1000 ${theme.bg} ${theme.text}`}>

      {/* BACKGROUND EXECUTIVO SUTIL */}
      <div className="absolute inset-0 pointer-events-none">
        <div className={`absolute inset-0 ${theme.grid} [background-size:32px_32px] opacity-40`}></div>
        <div className={`absolute top-0 left-0 w-full h-full bg-gradient-to-b ${isNight ? 'from-zinc-950 via-transparent to-zinc-950' : 'from-white via-transparent to-white'}`}></div>
      </div>

      <div className="max-w-7xl mx-auto px-6 relative z-10">

        {/* HEADER MINIMALISTA */}
        <div className={`mb-24 flex flex-col md:flex-row justify-between items-baseline gap-4 border-b ${theme.line} pb-12`}>
          <div>
          ...
          <p className={`text-xs ${theme.muted} max-w-[200px] leading-relaxed font-medium uppercase tracking-tighter`}>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {aircraft.map((plane, index) => (
            ...
            >
              <div className={`relative ${theme.cardBg} border shadow-[0_10px_50px_rgba(0,0,0,0.04)] overflow-hidden group backdrop-blur-sm`}>
                ...
                <div className="relative h-64 overflow-hidden bg-zinc-800">
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-between pointer-events-none">
                    <div className="flex justify-between items-start">
                      <div className={`${isNight ? 'bg-zinc-950/90' : 'bg-white/90'} backdrop-blur-md text-[9px] px-3 py-1 font-bold tracking-widest border ${isNight ? 'border-white/10 text-white' : 'border-zinc-200 text-zinc-800'} shadow-sm uppercase`}>
                        Unit_{plane.id}
                      </div>
                ...
                <div className="p-8">
                  ...
                  <h3 className={`text-2xl font-bold mb-8 tracking-tight ${theme.cardText}`}>
                    {plane.name}
                  </h3>
                      ...
                      <div key={spec.label} className="group/spec">
                        <div className="flex justify-between items-end mb-2">
                          <div className="flex items-center gap-3">
                            <spec.icon className="w-4 h-4 text-zinc-500" />
                            <span className={`text-[10px] ${theme.muted} uppercase font-bold tracking-widest`}>{spec.label}</span>
                          </div>
                          <span className={`text-sm font-bold italic ${theme.cardText}`}>{spec.value}</span>
                        </div>
                        <div className={`w-full h-[1px] ${isNight ? 'bg-white/5' : 'bg-zinc-100'} overflow-hidden`}>
                  ...
                  <button className={`w-full group/btn relative py-4 ${isNight ? 'bg-white text-zinc-950' : 'bg-zinc-950 text-white'} text-[10px] font-bold uppercase tracking-[0.3em] overflow-hidden transition-all shadow-xl ${isNight ? 'shadow-none' : 'shadow-zinc-200'}`}>
                ...
                <div className={`px-8 py-3 ${theme.footer} border-t flex justify-between items-center text-[8px] font-mono ${theme.muted}`}>
